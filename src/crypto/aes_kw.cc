...
bool aes_kw_wrap(const unsigned char* plaintext, size_t plaintext_len, const unsigned char* kek,
                 size_t kek_len, unsigned char* ciphertext_out, size_t* ciphertext_len) {
    ...
    do {
        if (EVP_EncryptInit_ex(ctx, EVP_aes_256_wrap(), nullptr, kek, nullptr) != 1)
            break;
...
bool aes_kw_unwrap(const unsigned char* ciphertext, size_t ciphertext_len, const unsigned char* kek,
                   size_t kek_len, unsigned char* plaintext_out, size_t* plaintext_len) {
    ...
    do {
        if (EVP_DecryptInit_ex(ctx, EVP_aes_256_wrap(), nullptr, kek, nullptr) != 1)
            break;
