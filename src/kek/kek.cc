...
bool kek_derive_wrapping_key(
    const unsigned char* auth_hash, size_t auth_hash_len,
    ...
    unsigned char* wrapping_key_out, size_t wrapping_key_len)
{
    ...
    return crypto_pwhash(
        wrapping_key_out, wrapping_key_len,
        reinterpret_cast<const char*>(auth_hash), auth_hash_len,
        salt,
...
bool kek_generate(
    const unsigned char* auth_hash, size_t auth_hash_len,
    unsigned char* wrapped_kek_out, size_t* wrapped_kek_len,
    unsigned char* salt_out, size_t* salt_len,
    char* expires_at_out, size_t expires_at_size)
{
    if (!auth_hash || !wrapped_kek_out || !wrapped_kek_len || !salt_out || !salt_len || !expires_at_out)
        return false;
    ...
    do {
        ...
        *salt_len = KEK_SALT_LEN;
        ...
        if (!kek_derive_wrapping_key(auth_hash, auth_hash_len,
                                     salt_out, KEK_SALT_LEN,
                                     wrapping_key, sizeof(wrapping_key)))
            break;

        if (!aes_kw_wrap(kek, sizeof(kek),
                         wrapping_key, sizeof(wrapping_key),
...
bool kek_unwrap(
    ...
    const unsigned char* auth_hash, size_t auth_hash_len,
    const unsigned char* salt, size_t salt_len,
    unsigned char* kek_out, size_t* kek_len)
{
    ...
    do {
        ...
        if (!aes_kw_unwrap(wrapped_kek, wrapped_kek_len,
                           wrapping_key, sizeof(wrapping_key),
                           kek_out, kek_len))
            break;
...
bool kek_rotate(sqlite3* db, int64_t user_id,
                const unsigned char* auth_hash, size_t auth_hash_len)
{
    if (!db || !auth_hash)
        return false;
    ...
    do {
        ...
        if (!kek_unwrap(old_kek.wrapped_kek.data(), old_kek.wrapped_kek.size(),
                        auth_hash, auth_hash_len,
                        old_kek.salt.data(), old_kek.salt.size(),
                        old_kek_raw, &old_kek_len))
            break;
        ...
        if (!aes_kw_wrap(new_kek, sizeof(new_kek),
                         new_wrapping_key, sizeof(new_wrapping_key),
        ...
        for (auto& secret : secrets)
        {
            ...
            if (!aes_gcm_encrypt(
                    plain_priv.data(), plain_priv.size(),
                    new_kek, sizeof(new_kek),
                    new_nonce, sizeof(new_nonce),
                    nullptr, 0,
            ...
            do {
                sqlite3_bind_blob(stmt, 1, new_priv.data(),
                                  static_cast<int>(new_priv.size()),
                                  SQLITE_TRANSIENT);
                if (!secret.public_key.empty())
                    sqlite3_bind_blob(stmt, 2, secret.public_key.data(),
                                      static_cast<int>(secret.public_key.size()),
                                      SQLITE_TRANSIENT);
                else
                    sqlite3_bind_null(stmt, 2);
                sqlite3_bind_blob(stmt, 3, new_nonce, sizeof(new_nonce),
                                  SQLITE_TRANSIENT);
                sqlite3_bind_blob(stmt, 4, new_priv_tag, sizeof(new_priv_tag),
                                  SQLITE_TRANSIENT);
