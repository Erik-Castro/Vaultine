...
TEST(AesGcmTest, EncryptDecryptRoundTrip)
{
    ...
    ASSERT_TRUE(aes_gcm_encrypt(
        reinterpret_cast<const unsigned char*>(plaintext), pt_len,
        key, sizeof(key),
        nonce, sizeof(nonce),
        nullptr, 0,
        ciphertext,
...
TEST(AesGcmTest, DifferentNonceProducesDifferentCiphertext)
{
    ...
    ASSERT_TRUE(aes_gcm_encrypt(
        reinterpret_cast<const unsigned char*>(plaintext), pt_len,
        key, sizeof(key), nonce_a, sizeof(nonce_a), nullptr, 0,
        ct_a, tag_a, sizeof(tag_a)));

    ASSERT_TRUE(aes_gcm_encrypt(
        reinterpret_cast<const unsigned char*>(plaintext), pt_len,
        key, sizeof(key), nonce_b, sizeof(nonce_b), nullptr, 0,
        ct_b, tag_b, sizeof(tag_b)));
...
TEST(AesGcmTest, AuthenticatedDataProtectsIntegrity)
{
    ...
    EXPECT_FALSE(aes_gcm_decrypt(
        ...
        key, sizeof(key),
        nonce, sizeof(nonce),
        reinterpret_cast<const unsigned char*>("wrong aad"), 9,
        tag, sizeof(tag),
        decrypted));
}
