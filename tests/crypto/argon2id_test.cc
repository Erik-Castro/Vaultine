...
TEST(Argon2idTest, HashAndVerifyRoundTrip)
{
    ...
    ASSERT_TRUE(argon2id_hash(
        reinterpret_cast<const unsigned char*>(password),
        std::strlen(password),
        salt, sizeof(salt),
        hash, sizeof(hash)));

    EXPECT_TRUE(argon2id_verify(
        reinterpret_cast<const unsigned char*>(password),
        std::strlen(password),
        hash, sizeof(hash)));
}
...
TEST(Argon2idTest, WrongPasswordFails)
{
    ...
    EXPECT_FALSE(argon2id_verify(
        reinterpret_cast<const unsigned char*>(wrong),
        std::strlen(wrong),
        hash, sizeof(hash)));
}
...
TEST(Argon2idTest, SamePasswordDifferentSaltsProduceDifferentHashes)
{
    ...
    ASSERT_TRUE(argon2id_hash(
        reinterpret_cast<const unsigned char*>(password),
        std::strlen(password),
        salt_a, sizeof(salt_a),
        hash_a, sizeof(hash_a)));

    ASSERT_TRUE(argon2id_hash(
        reinterpret_cast<const unsigned char*>(password),
        std::strlen(password),
        salt_b, sizeof(salt_b),
