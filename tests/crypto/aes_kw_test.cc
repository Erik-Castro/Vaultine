...
TEST(AesKwTest, WrapUnwrapRoundTrip) {
    ...
    ASSERT_TRUE(aes_kw_wrap(key, sizeof(key), kek, sizeof(kek), wrapped, &wrapped_len));
    EXPECT_EQ(wrapped_len, KEY_LEN + 8);

    unsigned char unwrapped[KEY_LEN];
    size_t unwrapped_len = 0;

    ASSERT_TRUE(aes_kw_unwrap(wrapped, wrapped_len, kek, sizeof(kek), unwrapped, &unwrapped_len));
...
TEST(AesKwTest, WrongKekFailsUnwrap) {
    ...
    ASSERT_TRUE(aes_kw_wrap(key, sizeof(key), kek_a, sizeof(kek_a), wrapped, &wrapped_len));

    unsigned char unwrapped[KEY_LEN];
    size_t unwrapped_len = 0;

    EXPECT_FALSE(
        aes_kw_unwrap(wrapped, wrapped_len, kek_b, sizeof(kek_b), unwrapped, &unwrapped_len));
}
...
TEST(AesKwTest, RejectsInvalidKeyLengths) {
    ...
    EXPECT_FALSE(
        aes_kw_wrap(key, sizeof(key), small_kek, sizeof(small_kek), wrapped, &wrapped_len));
}
...
TEST(AesKwTest, RejectsNonAlignedPlaintext) {
    ...
    EXPECT_FALSE(
        aes_kw_wrap(bad_input, sizeof(bad_input), kek, sizeof(kek), wrapped, &wrapped_len));
}
