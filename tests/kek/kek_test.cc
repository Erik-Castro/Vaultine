...
TEST_F(KekRotationTest, RotateGeneratesNewKek) {
    ...
    ASSERT_TRUE(kek_unwrap(new_row.wrapped_kek.data(), new_row.wrapped_kek.size(), auth_hash,
                           sizeof(auth_hash), new_row.salt.data(), new_row.salt.size(), new_kek,
                           &new_kek_len));
    ...
    ASSERT_TRUE(aes_gcm_decrypt(updated.private_key.data(), updated.private_key.size(), new_kek,
                                sizeof(new_kek), updated.nonce.data(), updated.nonce.size(),
                                nullptr, 0, updated.tag.data(), updated.tag.size(),
                                decrypted.data()));
...
TEST_F(KekRotationTest, RotateMultipleSecrets) {
    ...
    for (int i = 0; i < 3; ++i) {
        ...
        ASSERT_TRUE(aes_gcm_decrypt(sr.private_key.data(), sr.private_key.size(), new_kek,
                                    sizeof(new_kek), sr.nonce.data(), sr.nonce.size(), nullptr, 0,
                                    sr.tag.data(), sr.tag.size(), dec.data()));
    }
}
...
TEST_F(KekRotationTest, RotateWithPublicKey) {
    ...
    ASSERT_TRUE(secrets_store(db_, user_id, "keypair", priv_ct, sizeof(priv_ct), pub, sizeof(pub),
                              nonce, sizeof(nonce), priv_tag, sizeof(priv_tag),
                              "keypair with pub"));
    ...
    ASSERT_TRUE(aes_gcm_decrypt(sr.private_key.data(), sr.private_key.size(), new_kek,
                                sizeof(new_kek), sr.nonce.data(), sr.nonce.size(), nullptr, 0,
                                sr.tag.data(), sr.tag.size(), dec_priv.data()));
